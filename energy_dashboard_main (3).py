...
# ============================================================================
    ...
    fig = px.scatter(recent7.iloc[::12], x='net_load', y='price', color='price',
        color_continuous_scale='RdYlGn_r', trendline="lowess",
        labels={'net_load':'Net Load (MW)', 'price':'$/MWh'})
    fig.update_layout(**plotly_dark_layout(h=380))
...
# ============================================================================
    ...
    try:
        with open('/mnt/user-data/outputs/energy_trading_terminal_v2.html', 'r') as f:
            terminal_html = f.read()
        
        # Inject into streamlit — strip html/head/body tags for component injection
        import re
        # Extract just the style + body content for iframe
        st.components.v1.html(terminal_html, height=820, scrolling=False)
        
    except FileNotFoundError:
        # Fallback: show key trading metrics + charts inline
        st.warning("Trading terminal file not found. Showing inline trading view.")
        df = generate_sample_data(days=7)
